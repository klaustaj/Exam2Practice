...
class Box(object):
    ...
    def append_string(self, additional_contents):
        ...
        if len(additional_contents) <= space_remaining:
            ...
            leftovers = ''
            for k in range(len(additional_contents) - space_remaining):
                leftovers = leftovers+additional_contents[k+space_remaining]
    ...
    def shrink(self, new_volume):
        ...
        if len(self.contents) <= self.volume:
            ...
            num_extras = len(self.contents) - self.volume
            extras = ''
            for k in range(num_extras):
                extras = extras + self.contents[k + self.volume]
